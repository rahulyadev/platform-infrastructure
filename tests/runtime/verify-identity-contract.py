...
require(nginx.count("listen 80 default_server;") == 1)
require(nginx.count("listen [::]:80 default_server;") == 1)
require(nginx.count("listen 443 ssl default_server;") == 1)
require(nginx.count("listen [::]:443 ssl default_server;") == 1)
require("ssl_session_cache shared:TLS:10m;" in nginx)
require("ssl_session_timeout 1d;" in nginx)
require('readonly nginx_configuration="$(rooted /etc/nginx/conf.d/portfolio.conf)"' in deploy)
